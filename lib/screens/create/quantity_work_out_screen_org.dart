...
class _WorkoutQuantityScreenState extends State<WorkoutQuantityScreen> {
  ...
  void initVideo()  {
    ...
    video = ref.child("training_videos").child("${widget.workout.id}.mp4");
    print(video);
    // result?.then((value) => {
    //   videos = value.items.toList(),
    //
    //     }).whenComplete(() async => {
    //   for (var item in videos)
    //     {
    //       if (_getName(item.name) == widget.workout.id.toString())
    //         {
    //           _controller = VideoPlayerController.network(await item.getDownloadURL())
    //             ..initialize().then((_) =>
  ...
  Widget build(BuildContext context) {
    // final item = ModalRoute.of(context)?.settings.arguments as Map<String, Object>;
    // final title = item['name'] as String;
    // final workoutModel = item['exercise'] as WorkOutModel;
